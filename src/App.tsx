...
const roleTabs: Record<UserRole, string[]> = {
  ...
  supplier:   ['dashboard', 'register', 'submitBid', 'myContracts', 'disputes', 'reputation'],
  citizen:    ['dashboard', 'audit', 'whistleblower', 'reputation'],
  auditor:    ['dashboard', 'audit', 'supplier', 'reputation', 'dao'],
  oversight:  ['dashboard', 'audit', 'supplier', 'dao', 'reputation'],
};
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 0 }}>
            ...
            >
              {visibleTabs.map((tab, index) => {
                ...
                const navKey = tab.id === 'dashboard' ? (userRole === 'citizen' ? 'home' : 'myDashboard') : tab.id === 'pre' ? 'preTender' : tab.id === 'tender' ? 'tendering' : tab.id === 'post' ? 'postTender' : tab.id === 'audit' ? (userRole === 'auditor' ? 'auditRecords' : 'publicAudit') : tab.id === 'dao' ? 'daoGovernance' : tab.id === 'supplier' ? 'supplierTracker' : tab.id === 'submitBid' ? 'submitBid' : tab.id === 'myContracts' ? 'myContracts' : tab.id === 'disputes' ? 'disputes' : tab.id;
                return (
                  <button
                    key={tab.id}
                    role="tab"
